...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendarDays = document.querySelectorAll('span.fc-day-number');
    ...
    var importantEditButton = document.querySelectorAll('.importantTask li:last-of-type');
    ...
    // WHen youc click on a day of the calendar
    function calendarDayOnClick(info) 
    {
        console.log(info);
        /*var calendarDay = document.querySelector(.info);
    ...
    {
        content[index].classList.toggle('notshow');
        options[index].classList.toggle('notshow');
    }
    ...
    function changeDate(date) {
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                {
                    waitingSection.innerHTML += "<p> Vous n'avez rien de prévu pour ce jour.</p>";
                }
                else 
                {
                    for(let i = 0; i < waitingTasks.length; i++ )
                    {
                        waitingSection.innerHTML += 
                            '<article class="task flex-row">' +
                                '<a href="changeStatus&id=' + waitingTasks[i].id + '&status=failed&priority=' + waitingTasks[i].priority + '"><i class="fas fa-times"></i></a>' +
                                '<div>' +
                                    '<h4>' + waitingTasks[i].title + '</h4>' +
                                    '<p class="notshow">'+ waitingTasks[i].content + '</p>' +
                                    '<ul class="notshow flex-row">' +
                                        '<li> <a href="deleteTask&id=' + waitingTasks[i].id + '"><i class="fas fa-trash-alt"></i></a></li>' + 
                                        '<li data-id="' + waitingTasks[i].id + '"><i class="fas fa-edit"></i></li>' +
                                    '</ul>' +
                                '</div>' +
                                '<a href="changeStatus&id=' + waitingTasks[i].id + '&status=done&priority=' + waitingTasks[i].priority + '"><i class="fas fa-check"></i></a>' +
                            '</article>';
                    }
                } 
                        ...
                        doneSection.innerHTML += 
                            '<article class="task flex-row">' +
                                '<div>' +
                                    ...
                                    '<p class="notshow">'+ doneTasks[i].content + '</p>' +
                ...
                if (failedTasks.length !== 0)
                {
                    ...
                    for(let i = 0; i < failedTasks.length; i++ )
                    {
                        failedSection.innerHTML += 
                            '<article class="task flex-row">' +
                                '<div>' +
                                    '<h4>' + failedTasks[i].title + '</h4>' +
                                    '<p class="notshow">'+ failedTasks[i].content + '</p>' +
                                    '<ul class="notshow flex-row"></ul>' +
                                '</div>' +
                            '</article>';
                    }
